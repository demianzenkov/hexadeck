...
#define _MIDI_ITF_NBR 					0x00U
#define _MIDI_STR_DESC_IDX 				0x00U

uint8_t MIDI_IN_EP = MIDI_EPIN_ADDR;
uint8_t MIDI_OUT_EP = MIDI_EPOUT_ADDR;
                               ...
                               uint8_t cfgidx)
{
  ...
  pdev->pClassData_MIDI = USBD_malloc(sizeof (USBD_MIDI_HandleTypeDef));
  ...
  {
    ((USBD_MIDI_HandleTypeDef *)pdev->pClassData_MIDI)->state = MIDI_IDLE;
  }
  return ret;
}
                                 ...
                                 uint8_t cfgidx)
{
  ...
  if(pdev->pClassData_MIDI != NULL)
  {
    USBD_free(pdev->pClassData_MIDI);
...
{
  return ((USBD_MIDI_HandleTypeDef *)pdev->pClassData_MIDI)->state;
}
